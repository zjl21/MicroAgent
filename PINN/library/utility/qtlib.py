...
def extract_block_torch_vec(data: torch.Tensor, inds: np.ndarray) -> torch.Tensor:
    """
    Vectorized patch extraction using PyTorch advanced indexing.
    Works on both CPU and GPU tensors — no Python loop, no numpy allocation.

    Parameters
    ----------
    data : (H, W, D, C) torch.Tensor
    inds : (N, 6) int ndarray  [x0, x1, y0, y1, z0, z1]  (inclusive)

    Returns
    -------
    blocks : (N, P, P, P, C) torch.Tensor, same device as data
    """
    P   = int(inds[0, 1] - inds[0, 0] + 1)
    N   = inds.shape[0]
    dev = data.device

    # origins: (N, 3) — x0, y0, z0 for each patch
    origins = torch.as_tensor(inds[:, [0, 2, 4]], dtype=torch.long, device=dev)

    # offsets: (P,)
    off = torch.arange(P, dtype=torch.long, device=dev)

    # absolute indices per axis: broadcast to (N, P, P, P)
    ix = (origins[:, 0:1] + off).reshape(N, P,  1,  1).expand(N, P, P, P)
    iy = (origins[:, 1:2] + off).reshape(N,  1, P,  1).expand(N, P, P, P)
    iz = (origins[:, 2:3] + off).reshape(N,  1,  1, P).expand(N, P, P, P)

    # single gather: (N, P, P, P, C)
